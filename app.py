...
# For displaying unique class names without duplicates and underscores
unique_class_names = ['Apple', 'Blueberry', 'Cherry (including sour)', 'Corn (maize)', 'Grape', 'Orange', 'Peach', 'Pepper, bell', 'Potato', 'Raspberry', 'Soybean', 'Squash', 'Strawberry', 'Tomato']
...
def upload_predict():
    if request.method == 'POST':
        if 'file' not in request.files:
            return render_template('index.html', prediction="No image uploaded.", remedy=None, classes=unique_class_names)
        
        file = request.files['file']
        if not file:
            return render_template('index.html', prediction="No image selected.", remedy=None, classes=unique_class_names)
        ...
        return render_template('index.html', prediction=prediction, remedy=remedy, classes=unique_class_names)
    
    # logger.info("problem in rendering uploaded page.")
    return render_template('index.html', prediction=None, remedy=None, classes=unique_class_names)
...
def capture_predict():
    ...
    if not file:
        return render_template('index.html', prediction="No image captured.", remedy=None, classes=unique_class_names)
